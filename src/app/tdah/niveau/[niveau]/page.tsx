...
export function generateStaticParams() {
  return NIVEAUX
    .filter((n) => adaptedMatieresByProfilNiveau("tdah", n.slug).length > 0)
    .map((n) => ({ niveau: n.slug }));
}

export default async function TdahNiveauPage({ params }: { params: Promise<{ niveau: string }> }) {
  const { niveau: niveauSlug } = await params;
  const niveau = NIVEAUX.find((n) => n.slug === niveauSlug);
  if (!niveau) notFound();

  const info = PROFIL_INFO.tdah;
  const matiereSlugs = adaptedMatieresByProfilNiveau("tdah", niveau.slug);
            ...
            {matieres.length > 0 ? (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {matieres.map((matiere) => {
                  const nbChapitres = adaptedLessonsByProfilNiveauMatiere("tdah", niveau.slug, matiere.slug).length;
                  return (
                    <Link
                      key={matiere.slug}
                      href={`/tdah/niveau/${niveau.slug}/${matiere.slug}`}

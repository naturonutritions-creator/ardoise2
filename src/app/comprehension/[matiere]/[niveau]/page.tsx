...
import { TEXTES_COMPREHENSION, textesComprehension } from "@/content/comprehension";
import { BookOpenCheck, Clock } from "lucide-react";

export function generateStaticParams() {
  const pairs = new Set(TEXTES_COMPREHENSION.map((t) => `${t.matiere}__${t.niveau}`));
...
export default async function ComprehensionListPage({
  params,
}: {
  ...
  const textes = textesComprehension(matiereSlug, niveauSlug);
        ...
        <section className="bg-ardoise-900 py-14 text-craie">
          <Container>
            ...
            <p className="mt-3 max-w-2xl text-craie/80">
              Lis chaque texte (tu peux l&apos;écouter avec la bonne prononciation), puis réponds
              aux questions pour vérifier ta compréhension.
            </p>
            <Link
            ...
            {textes.length > 0 ? (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {textes.map((t) => (
                  <Link
                    key={t.slug}
                    href={`/comprehension/${matiere.slug}/${niveau.slug}/${t.slug}`}
              ...
              <p className="text-sm text-ardoise-700/60">
                De nouveaux textes de compréhension pour ce niveau arrivent bientôt.
              </p>
            )}
          </Container>
        </section>
